    ...
    def register_sync(self, database, config_id, session_id, config_ids, sync_multi_session_offline):
        ...
        if sync_multi_session_offline:
            ...
            if len(log_ids) > 0:
                if len(log_ids) == 1:
                    log_ids.append(0)
                request.env.cr.execute("UPDATE pos_sync_session_log SET state='restored' WHERE id in %s",
                                       (tuple(log_ids),))
                request.env.cr.commit()
                _logger.info('new notifications: %s' % len(log_ids))
            ...
            if request.env.context.get('uid', None) and send_from_session:
                sql_from = "INSERT INTO " \
                           ...
                           "VALUES (%s, %s, 'ready', '%s', '%s')" % (
                               send_from_session.id,
                               request.env.context.get('uid', None) if request.env.context.get('uid', None) else 'Null',
                               message.get('action', None) if message.get('action', None) else 'Null',
                               json.dumps(message)
                           )
                ...
                for send_to_session in send_to_sessions:
                    sql_to = "INSERT INTO " \
                             ...
                             "VALUES (%s, %s, %s, %s, 'ready', '%s', '%s')" % (
                                 send_from_session.id,
                                 send_to_session.id,
                                 request.env.context.get('uid', None) if request.env.context.get('uid',
                                                                                                 None) else 'Null',
                                 send_to_session.user_id.id,
                                 message.get('action', None) if message.get('action', None) else 'Null',
                                 json.dumps(message)
                             )
